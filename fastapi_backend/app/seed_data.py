...
def _seed_ingredientes(session: Session):
    ...
    ingredientes = [
        ("Masa fresca", "Masa artesanal preparada diariamente", 100.0, um_pieza.id, False),
        ("Salsa de tomate", "Salsa casera con tomates frescos y albahaca", 50.0, um_pieza.id, False),
        ("Queso mozzarella", "Mozzarella fresca de búfala italiana", 80.0, um_gramo.id, False),
        ("Pepperoni", "Rodajas de pepperoni ahumado premium", 40.0, um_gramo.id, False),
        ("Jamón cocido", "Jamón cocido natural en fetas finas", 35.0, um_gramo.id, False),
        ("Champiñones", "Champiñones frescos fileteados", 30.0, um_gramo.id, False),
        ("Cebolla", "Cebolla morada en aros finos", 25.0, um_gramo.id, False),
        ("Pimiento verde", "Pimiento verde fresco en tiras", 25.0, um_gramo.id, False),
        ("Aceitunas negras", "Aceitunas negras descarozadas", 20.0, um_gramo.id, False),
        ("Albahaca fresca", "Hojas de albahaca fresca orgánica", 15.0, um_gramo.id, False),
        ("Queso parmesano", "Parmesano reggiano rallado", 30.0, um_gramo.id, False),
        ("Tomate cherry", "Tomates cherry maduros en mitades", 25.0, um_gramo.id, False),
        ("Rúcula", "Rúcula fresca orgánica", 15.0, um_gramo.id, False),
        ("Prosciutto", "Prosciutto di Parma en fetas finas", 25.0, um_gramo.id, False),
        ("Anchoas", "Filetes de anchoas en aceite de oliva", 15.0, um_gramo.id, False),
        ("Carne molida", "Carne de res molida sazonada", 50.0, um_gramo.id, False),
        ("Tocino", "Tocino ahumado en tiras crocantes", 30.0, um_gramo.id, False),
        ("Piña", "Piña natural en cubos pequeños", 25.0, um_gramo.id, False),
        ("Queso cheddar", "Queso cheddar maduro rallado", 30.0, um_gramo.id, False),
        ("Orégano", "Orégano seco importado de Grecia", 5.0, um_gramo.id, False),
        ("Ajo", "Ajo fresco picado finamente", 10.0, um_gramo.id, False),
        ("Salsa BBQ", "Salsa barbacoa ahumada artesanal", 20.0, um_ml.id, False),
        ("Queso gorgonzola", "Queso gorgonzola DOP italiano", 25.0, um_gramo.id, True),
        ("Queso fontina", "Queso fontina suave del Valle de Aosta", 25.0, um_gramo.id, False),
        ("Huevo", "Huevo fresco de campo", 15.0, um_pieza.id, False),
        ("Pollo", "Pechuga de pollo grillada en tiras", 40.0, um_gramo.id, False),
        ("Palmitos", "Palmitos frescos en rodajas", 20.0, um_gramo.id, False),
        ("Morrón asado", "Morrón rojo asado pelado en tiras", 20.0, um_gramo.id, False),
        ("Mascarpone", "Queso mascarpone cremoso italiano", 20.0, um_gramo.id, False),
        ("Café espresso", "Café espresso italiano intenso", 0, um_ml.id, False),
        ("Cacao en polvo", "Cacao amargo en polvo sin azúcar", 0, um_gramo.id, False),
        ("Vainilla", "Esencia de vainilla natural de Madagascar", 0, um_ml.id, False),
        ("Helado de crema", "Helado artesanal de crema americana", 0, um_gramo.id, False),
        ("Chocolate", "Chocolate semi-amargo 70% cacao", 0, um_gramo.id, False),
        ("Nueces", "Nueces pecanas tostadas", 0, um_gramo.id, False),
        ("Pan de ajo", "Pan ciabatta con manteca de ajo y perejil", 0, um_pieza.id, False),
        ("Limón", "Limones frescos orgánicos", 0, um_pieza.id, False),
        ("Azúcar", "Azúcar orgánica de caña", 0, um_gramo.id, False),
        ("Menta", "Hojas de menta fresca", 0, um_gramo.id, False),
        ("Miel", "Miel pura de abeja orgánica", 0, um_ml.id, False),
    ]

    for nombre, desc, stock, um_id, alergeno in ingredientes:
