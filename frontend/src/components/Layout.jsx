...
const roleBadgeColors = {
  ...
  PEDIDOS: 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300',
}

const sidebarItems = [
  { path: '/', label: 'Dashboard', icon: 'grid', roles: ['ADMIN', 'STOCK', 'PEDIDOS'] },
  { path: '/usuarios', label: 'Usuarios', icon: 'users', roles: ['ADMIN'] },
  { path: '/productos', label: 'Productos', icon: 'box', roles: ['ADMIN', 'STOCK'] },
  { path: '/categorias', label: 'Categorías', icon: 'folder', roles: ['ADMIN'] },
  { path: '/ingredientes', label: 'Ingredientes', icon: 'leaf', roles: ['ADMIN'] },
  { path: '/pedidos', label: 'Pedidos', icon: 'clipboard', roles: ['ADMIN', 'PEDIDOS'] },
]
...
function Layout() {
  ...
  const breadcrumbs = (() => {
    ...
    for (const part of parts) {
      if (part.match(/^\d+$/) || part === 'nuevo' || part === 'nueva' || part === 'editar' || part === 'estado') {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      >
        {/* Logo */}
        <div className={`flex items-center h-16 px-4 border-b border-white/10 ${!sidebarOpen && 'lg:justify-center lg:px-2'}`}>
          <span className="text-xl flex-shrink-0">🍕</span>
          <span className={`ml-3 font-bold text-lg whitespace-nowrap transition-opacity duration-200 ${sidebarOpen ? 'opacity-100' : 'opacity-0 lg:hidden'}`}>
            Food Store
          </span>
        </div>
          ...
          {visibleItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm font-medium whitespace-nowrap transition-opacity duration-200 ${sidebarOpen ? 'opacity-100' : 'opacity-0 lg:hidden'}`}>
                  {item.label}
                </span>
              </Link>
            )
        ...
        {/* Bottom section */}
        <div className={`border-t border-white/10 p-3 space-y-2 ${!sidebarOpen && 'lg:p-2'}`}>
          <button
          ...
          >
            ...
            <span className={`text-sm whitespace-nowrap transition-opacity duration-200 ${sidebarOpen ? 'opacity-100' : 'opacity-0 lg:hidden'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex items-center justify-between h-16 px-4 lg:px-6 bg-white dark:bg-[#1c1917] border-b border-stone-200 dark:border-stone-800 flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${roleBadgeColors[role] || 'bg-stone-100 text-stone-800 dark:bg-stone-800 dark:text-stone-300'}`}>
              {role}
            </span>
          </div>
        </header>
